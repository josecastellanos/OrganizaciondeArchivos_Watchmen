...
dataFile::dataFile(char *_name, QTextEdit *log)
{
    ...
    log->append("- dataFile.h: "+temp+" ha sido abierto\n");
    ...
    int n=temp.indexOf('.',0,Qt::CaseSensitive);

    QString bname=temp.mid(0,n);
    QString avlname=temp.mid(0,n);

    bname.append("_btree.dat");
    avlname.append("_avl.dat");

    arbolB=new btree((char *)bname.toUtf8().constData(),log);
    arbolAVL=new avl((char *)avlname.toUtf8().constData(),log);
}
...
void dataFile::add(long id, double time, char *source, char *destino, char *protocolo, char *info)
{
    ...
    {
        log->append("- dataFile.h: No hay espacio para "+QString::number(id)+" o ya existe\n");
        disco.close();
        return;
    }
...
void dataFile::search(char *protocolo, QTableWidget *tb)
{
    ...
    log->append("- dataFile.h: Se ha buscado por "+QString(protocolo)+"\n");
